                                 ...
                                 double *pdTmpPeakExtracted)
{
   ...
   for (i=1; i<pScoring->_spectrumInfoInternal.iArraySize; i++)
   {
                  ...
                  || g_staticParams.ionInformation.iIonVal[ION_SERIES_Y]))
      {
          ...
          for (int losses : g_staticParams.ionInformation.ammoniaLosses) {
              if (losses != 0) {
                  iTmp = i - BIN((float)losses * g_staticParams.massUtility.dNH3);
                  if (iTmp >= 0) {
          ...
          for (int losses : g_staticParams.ionInformation.waterLosses) {
              if (losses != 0) {
                  iTmp = i - BIN((float)losses * g_staticParams.massUtility.dH2O);
                  if (iTmp >= 0) {
                      pScoring->pfFastXcorrDataNL[i] += (float)((pdTmpCorrelationData[iTmp] - pdTmpFastXcorrData[iTmp]) * 0.2);
                  }
              }
                               ...
                               struct PreprocessStruct *pPre)
{
   ...
   while(true)
   {
      ...
      if (firstPeak || dIon < pScoring->_spectrumInfoInternal.minMz) {
          pScoring->_spectrumInfoInternal.minMz = dIon;
      }

      if (firstPeak || dIon > pScoring->_spectrumInfoInternal.maxMz) {
          pScoring->_spectrumInfoInternal.maxMz = dIon;
      }
...
/**
  ...
  * Nothing to do with Selenocysteine.
  */
