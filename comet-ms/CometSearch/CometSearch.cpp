      ...
      while(!feof(fp))
      {
         ...
         if (iTmpCh == '>') // Expect a '>' for sequence header line.
         {
            ...
            if (g_staticParams.options.bCreateIndex)
            {
               ...
               g_pvProteinNames.insert(std::make_pair(sEntry.lProteinFilePosition, sEntry));
               ...
               while (iWhichQuery < (int)g_pvQuery.size())
               {
                  ...
                  if (CheckMassMatch(iWhichQuery, dCalcPepMass))
                  {
                     ...
                     if (bFirstTimeThroughLoopForPeptide && !g_staticParams.options.bCreateIndex)
                     {
                        ...
                        //fragmentCalculator.generateFragments(szProteinSeq + iStartPos, iLenPeptide, _pdAAforward, _pdAAreverse, binnedIonMasses);
                             ...
                             struct sDBEntry *dbe)
{
   ...
   //
   //
   FragmentModelData fragmentData;
   fragmentData.nHeavy = round(pQuery->_pepMassInfo.dExpPepMass - dCalcPepMass);
   fragmentData.obsCharge = pQuery->_spectrumInfoInternal.iChargeState;
   fragmentData.maxCharge = g_massRange.iMaxFragmentCharge;
   fragmentData.aaFwdMasses = _pdAAforward;
   fragmentData.aaRevMasses = _pdAAreverse;
   ...
   {
     if (ctCharge == 1 && pQuery->ppfSparseFastXcorrDataNL)
     {
        ppSparseFastXcorrData = pQuery->ppfSparseFastXcorrDataNL;
     }
     else
     ...
     int x,y;
     for (auto frag : binnedIonMasses.at(ctCharge))
     {
        int bin = BIN(frag.mz);
        //MH: newer sparse matrix converts bin to sparse matrix bin
        x = bin / SPARSE_MATRIX_SIZE;
        ...
        dXcorr += ppSparseFastXcorrData[x][y] * frag.intensity;
     }
   ...
   while (iWhichQuery < (int)g_pvQuery.size())
   {
      ...
      if (CheckMassMatch(iWhichQuery, dCalcPepMass))
      {
         ...
         if (bFirstTimeThroughLoopForPeptide)
         {
            ...
            //fragmentCalculator.generateFragments(szProteinSeq + _varModInfo.iStartPos, iLenPeptide, _pdAAforward, _pdAAreverse, binnedIonMasses);
