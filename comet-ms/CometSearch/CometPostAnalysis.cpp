...
//             pOutput[i].pWhichProtein.end());
      ...
      if (pOutput[i].iLenPeptide>0) // take care of possible edge case
      {
         ...
         fragmentData.nHeavy = round(pQuery->_pepMassInfo.dExpPepMass - pOutput[i].dPepMass);
         fragmentData.obsCharge = pQuery->_spectrumInfoInternal.iChargeState;
         fragmentData.maxCharge = g_massRange.iMaxFragmentCharge;
         fragmentData.aaFwdMasses = pdAAforward;
         fragmentData.aaRevMasses = pdAAreverse;
         fragmentData.minMz = pQuery->_spectrumInfoInternal.minMz;
         fragmentData.maxMz = pQuery->_spectrumInfoInternal.maxMz;

         FragmentCalculator& fragmenter = FragmentCalculator::instance();
         vector<vector<FragmentIon> > fragments;
         fragmenter.generateFragments(
                     pOutput[i].szPeptide,
                     pOutput[i].iLenPeptide,
                     fragmentData,
                     fragments);

         int iMatchedFragmentIonCt = 0;
         int iTotalFragmentIonCt = 0;
         for (ctCharge=1; ctCharge<=iMaxFragCharge; ctCharge++)
         {
           for (auto frag : fragments.at(ctCharge))
           {
             float fSpScore = FindSpScore(pQuery,BIN(frag.mz));
         ...
         pOutput[i].iTotalIons = iTotalFragmentIonCt;
      }
   }
}
            ...
            for (ctCharge=1; ctCharge<=iMaxFragCharge; ctCharge++)
            {
               ...
               if (dFragmentIonMass < pQuery->_pepMassInfo.dExpPepMass)
               {
                  ...
                  if (iFragmentIonMass < pQuery->_spectrumInfoInternal.iArraySize && iFragmentIonMass >= 0)
                  {
                     ...
                     if (x <= iMax && pQuery->ppfSparseFastXcorrData[x] != NULL)
                     {
                                     ...
                                     int bin)
{
   ...
   if (x > iMax || pQuery->ppfSparseSpScoreData[x] == NULL)
      return 0.0f;
