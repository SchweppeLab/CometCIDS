...
//       exit(0);
       ...
       if (argc >= 3 && strcmp(argv[2], "--genTheoreticalSpectrum") == 0) {
           ...
           int obsCharge = std::atoi(argv[5]);
           int numDeuteria = std::atoi(argv[6]);

           FragmentModelData data;
           data.nHeavy = std::atoi(argv[6]);
           ...
           manager->InitializeStaticParams();
           ...
           g_massRange.iMaxFragmentCharge = g_staticParams.options.iMaxFragmentCharge;

           //mimics CometSearch.cpp#2870
           data.maxCharge = g_massRange.iMaxFragmentCharge;
           ...
           //OLD - not working properly for mods (uses hard-coded constants)
           vector<double> fwd = generateFwd(peptideSeq);
           vector<double> rev = generateRev(peptideSeq);

           //NEW
           // =============================================================== //

           //taken from CometSearch::AnalyzeIndexPep()
...
//           }
           ...
           //fragmentCalculator.generateFragments(sDBI.szPeptide, iEndPos, _pdAAforward, _pdAAreverse, binnedIonMasses);

           // =============================================================== //

           data.aaFwdMasses = &fwd.at(0);
           data.aaRevMasses = &rev.at(0);

           std::strcpy(cstr, peptideSeq.c_str());
...
void Usage(char *pszCmd)
{
   ...
   logout("\n");
   logout(" Alternative usage for mac os x testing: ./comet.exe --osxtm [options]\n");
   logout("\n");
   logout("       options:\n\n");
   logout("       --isoDist <peptide_sequence> <number of deuteria>\n");
   logout("              to print out theoretical intensity distributions for <peptide_sequence>, sqrt normalized\n");
   logout("\n");
   logout("       --genTheoreticalSpectrum <paramsFile> <peptide_sequence> <peptide_charge> <number_deuteria>\n");
   logout("              generate theoretical m/z, intensity values for <peptide_sequence>\n");
   logout("              respects parameters in <paramsFile>\n");
   logout("\n");
      ...
      if (fgets(szParamBuf, SIZE_BUF, fp) != NULL)
      {
         ...
         if ( (pStr = strchr(szParamBuf, '=')) != NULL)
         {
            ...
            *pStr = 0;                     // Null rest of szParamName at equal char.
            ...
            else if (!strcmp(szParamName, "water_losses"))
            {
                // Remove white spaces at beginning/end of szParamVal
                int iLen = (int)strlen(szParamVal);
                char *szTrimmed = szParamVal;
                ...
                char szOption[512];
                strcpy(szOption, szParamVal);
                pSearchMgr->SetParam("water_losses", szOption, szOption);
            }
            else if (!strcmp(szParamName, "ammonia_losses"))
            {
                // Remove white spaces at beginning/end of szParamVal
                int iLen = (int)strlen(szParamVal);
                char *szTrimmed = szParamVal;
                ...
                char szOption[512];
                strcpy(szOption, szParamVal);
                pSearchMgr->SetParam("ammonia_losses", szOption, szOption);
            }
...
*/
            ...
            else if(strstr(szParamName, "dist-")) {
                pSearchMgr->SetParam(szParamName, szParamVal, szParamVal);
            }
            else if(strstr(szParamName, "relative_minimum_intensity")) {
                pSearchMgr->SetParam(szParamName, szParamVal, szParamVal);
            }
            else if(strstr(szParamName, "obs_charge_offset")) {
                pSearchMgr->SetParam(szParamName, szParamVal, szParamVal);
            }
            else if(strstr(szParamName, "model_sqrt")) {
                pSearchMgr->SetParam(szParamName, szParamVal, szParamVal);
            }
            else if(strstr(szParamName, "model_windowed_scaling")) {
                pSearchMgr->SetParam(szParamName, szParamVal, szParamVal);
            }
            else if (strstr(szParamName, "isotope_sqrt")) {
                pSearchMgr->SetParam(szParamName, szParamVal, szParamVal);
            }
            else if(strstr(szParamName, "normalization")) {
                pSearchMgr->SetParam(szParamName, szParamVal, szParamVal);
            }
            else if(strstr(szParamName, "use_mz_range")) {
                pSearchMgr->SetParam(szParamName, szParamVal, szParamVal);
            }
